    ...
    <>
      ...
      <section id="work-timeline">
        <div className="flex items-baseline gap-4 mb-10">
          <span className="font-mono text-xs text-moss tracking-[0.2em]">05</span>
          <h1 className="font-serif text-2xl lg:text-3xl font-medium text-paper">Work Timeline</h1>
          <span className="flex-1 h-px bg-line ml-2" />
        </div>

        <p className="text-bone/80 mb-14 leading-relaxed text-sm max-w-2xl">
        ...
        <motion.div
          ...
          className="mt-4 ml-3 pl-10 flex items-start gap-3 text-bone/80"
        >
          <GraduationCap size={16} aria-hidden="true" className="mt-0.5 flex-shrink-0 text-bone/70" />
          <p className="text-xs leading-relaxed">
            <span className="text-paper">{education.degree}</span> — {education.school}
    ...
    >
      ...
      <div className="group border border-line bg-ink-2 p-6 lg:p-8 transition-colors hover:border-moss/60">
        <div className="flex flex-wrap items-center gap-3 mb-2">
          <span className="font-mono text-xs text-moss uppercase tracking-wide font-semibold">
            {entry.duration}
          </span>
          {entry.current && (
        ...
        <h2 className="font-serif text-xl lg:text-2xl font-medium text-paper">
          {entry.company}
        </h2>
        {entry.subtitle && (
          <p className="text-bone/80 text-xs mt-1 italic">{entry.subtitle}</p>
        )}
        <p className="font-mono text-xs text-moss uppercase tracking-wide mt-2 mb-4">
          {entry.role}
        </p>

        <p className="text-bone/80 leading-relaxed text-sm mb-5">{entry.description}</p>

        <ul className="space-y-2.5">
          {entry.highlights.map((highlight, i) => (
            <li key={i} className="flex items-start gap-3 text-sm text-bone/80 leading-relaxed">
              <span aria-hidden="true" className="mt-2 w-1 h-1 rounded-full bg-moss/70 flex-shrink-0" />
              <span>{highlight}</span>
            </li>
          ))}
        </ul>
  ...
  return (
    <li className="relative">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-1">
        <span className="font-mono text-[11px] text-moss uppercase tracking-wide">
          {project.duration}
        </span>
        {project.current && (
        ...
        {!project.current && project.maintaining && (
          <span className="font-mono text-[9px] uppercase tracking-wide text-moss/90 border border-moss/25 px-1.5 py-0.5">
            Live — Maintaining
          </span>
        )}
      </div>

      <h3 className="font-serif text-base font-medium text-paper">{project.name}</h3>

      <p className="text-bone/80 text-sm leading-relaxed mt-1">{project.description}</p>
